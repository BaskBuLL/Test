...
/**
 * @author liuzhuo
 * @date 2022/3/9 3:42 下午
 */
public class 单个业务扩展点调用失败率较高 {
    ...
    public static void main(String[] args) {
        ...
        metricTemplate.setName("单个业务扩展点调用失败率较高");
        ...
        qpm.setName("每分钟调用次数");
        qpm.setQueryMql("columbus.biz_ext_qpm{appName=\"{{ appName }}\"}${timeRange:TIME_RANGE} | time_aggr<${slidingWindow:SLIDING_WINDOW}, sum> | tag_aggr<[\"interfaceName\"], sum>");
        qpm.setTargetTagName("appName");
        qpm.setDetectionMethod("THRESHOLD");
        qpm.setComparisonOperator("GTE");
        qpm.setRightVal(Double.valueOf(5));
        qpm.setQueryValFormat(0);
        qpm.setQueryValCompareStrategy(0);

        failCount.setName("每分钟失败次数");
        failCount.setQueryMql("columbus.biz_ext_qpm{resultType=\"EXCEPTION\",appName=\"{{ appName }}\"}${timeRange:TIME_RANGE} | time_aggr<${slidingWindow:SLIDING_WINDOW}, sum> | tag_aggr<[\"interfaceName\"], sum>");
        ...
        failRate.setQueryMql("columbus.request_fail_rate{appName=\"{{ appName }}\"}${timeRange:TIME_RANGE} | time_aggr<${slidingWindow:SLIDING_WINDOW}, sum> | tag_aggr<[\"interfaceName\"], sum>");
        failRate.setTargetTagName("appName");
        failRate.setDetectionMethod("THRESHOLD");
        failRate.setComparisonOperator("GTE");
        failRate.setRightVal(0.25);
